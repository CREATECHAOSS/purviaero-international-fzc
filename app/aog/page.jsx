    ...
    <main className="bg-primary">
      ...
      <section className="relative overflow-hidden pt-24 pb-32 md:pt-32 md:pb-40 border-b border-white/5">
        ...
        <div 
          ...
          style={{ background: "radial-gradient(ellipse at center, rgba(0,113,206,0.15) 0%, transparent 70%)", transform: "translate(30%, -30%)" }}
        />

        <div className="relative max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <FadeIn className="flex flex-col items-start max-w-3xl">
            <span className="inline-flex items-center gap-3 bg-secondary/20 text-white text-[10px] font-bold uppercase tracking-[0.25em] px-5 py-2.5 rounded-full mb-10 border border-secondary/40 backdrop-blur-md">
            ...
            <Link
              href="/contact"
              className="mt-12 btn-primary px-12 py-5 text-lg shadow-2xl shadow-secondary/20"
            >
      ...
      <section className="relative py-20 md:py-32 bg-white rounded-t-[3rem] -mt-12 z-10 shadow-[0_-20px_50px_rgba(0,0,0,0.2)]">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <FadeIn className="text-center mb-16 md:mb-24 max-w-2xl mx-auto">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-primary tracking-tight">Rapid Response Protocol</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {protocol.map((item, idx) => (
              <FadeIn key={item.step} delay={idx * 150} className="relative group">
                <div className="bg-[#F9FAFB] border border-primary/5 p-12 rounded-[2.5rem] transition-all duration-500 hover:bg-white hover:shadow-2xl hover:border-secondary/20 h-full">
                  <span className="text-6xl font-black text-primary/5 absolute top-8 right-8 select-none group-hover:text-secondary/10 transition-colors pointer-events-none italic">
                    {item.step}
                  </span>
                  <h4 className="font-bold text-primary text-2xl mb-5 tracking-tight">{item.title}</h4>
          ...
          <FadeIn delay={400} className="mt-20 md:mt-32 bg-primary rounded-[2.5rem] p-10 md:p-20 relative overflow-hidden group border border-white/5 shadow-2xl">
            <div className="absolute top-0 right-0 w-96 h-96 bg-secondary/10 blur-[100px] rounded-full translate-x-1/2 -translate-y-1/2 group-hover:bg-secondary/20 transition-all duration-700" />
            <div className="relative z-10 flex flex-col lg:grid lg:grid-cols-2 lg:items-center gap-12">
              <div>
                <span className="block text-secondary text-xs font-bold uppercase tracking-[0.3em] mb-4">Direct Intake Portal</span>
              ...
              <div className="lg:flex lg:flex-col lg:items-end gap-6">
                <Link
                  href="/contact"
                  className="btn-primary px-12 py-6 text-xl shadow-2xl shadow-secondary/50 w-full lg:w-auto text-center"
                >
                  Submit Priority RFQ
                </Link>
